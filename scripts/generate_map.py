...
# ===== Google Drive 認証 (個人 token.json を Base64 から読み込む) =====
token_info = json.loads(base64.b64decode(os.environ['SERVICE_ACCOUNT_B64']))
...
def get_file_bytes(file_id):
    ...
    downloader = MediaIoBaseUpload(fh, mimetype='image/png')  # ← upload用だけどdownloadは別
    # download処理
...
def create_popup(img, display_scale=2.0):
    img = img.copy()
    w,h = img.size
    img = img.resize((int(w*display_scale), int(h*display_scale)), Image.LANCZOS)
    return img
